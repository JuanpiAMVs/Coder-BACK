...
const getRestorePasswordView = async (req, res) => {
  ...
  const getProductsOnCart = async (req, res) => {
    try {
      ...
      const productsCart = productsArray.map((item) => {
        let product = item.product.toObject(); // Convert to a plain JavaScript object.
        delete product._id; // Delete the '_id' property.
        return {...product, quantity: item.quantity, productTotal: item.quantity * product.price}
      })
      console.log(productsCart)
      const grandTotal = productsCart.reduce((total, product) => total + product.productTotal, 0);
      res.render("cart",{
        css: 'cart',
